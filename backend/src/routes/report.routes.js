...
router.post('/export/:projectId', authenticate, async (req, res) => {
  try {
    ...
    if (format === 'pdf') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=qa-assist-report.pdf');
      ...
      csv += 'NOTES\n';
      csv += '=====\n';
      csv += 'Generated by QA Assist - Professional Test Management\n';
      csv += 'For more details, visit the QA Assist Dashboard\n';

      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', 'attachment; filename=qa-assist-report.csv');
      res.send(csv);
    } else {
      // JSON format
      res.setHeader('Content-Type', 'application/json');
      res.setHeader('Content-Disposition', 'attachment; filename=qa-assist-report.json');
      res.json(report);
    }
  } catch (err) {
